"""
v2b backtest engine — v2 + 3-month duration momentum gate on TLT/IEF.
"""
import logging
import pandas as pd
import config_v2b as config

from strategy_v2b.signals   import compute_all_macro, momentum, rolling_vol, resample_to_month_end, momentum_short
from strategy_v2b.portfolio import build_weight_series
...
def run(macro: pd.DataFrame, prices: pd.DataFrame) -> dict:
    ...
    mom_short_m = mom_short_m.reindex(common)

    weights = build_weight_series(macro_m, mom_m, vol_m, mom_short_monthly=mom_short_m)
