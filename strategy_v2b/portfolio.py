...
def build_weights(
    ...
    # ── 7a. Short-window gate — long-duration ETFs only (v2b) ────────────────
    # 3-month momentum catches rate-hike sell-offs faster than the 12-1 month
    # filter below. Only applied to TLT/IEF (not SHY, which is the cash bucket).
    if mom_short is not None:
        for etf in config.DURATION_MOM_GATE_ETFS:
            if etf in w.index and etf in mom_short.index and pd.notna(mom_short[etf]):
                if mom_short[etf] < 0:
                    w[etf] = 0.0
...
def build_weight_series(
    ...
    mom_short_monthly: "pd.DataFrame | None" = None,
) -> pd.DataFrame:
    records = []
    for date in macro_monthly.index:
        ...
        if "usd_z" in row.index and pd.notna(row["usd_z"]):
            ...
            usd_z = 0.0
        mom_short_row = (
            mom_short_monthly.loc[date]
            if mom_short_monthly is not None and date in mom_short_monthly.index
            else None
        )
        w = build_weights(
