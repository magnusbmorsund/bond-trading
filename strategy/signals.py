...
CREDIT composite (→ LQD/HYG allocation):
  s4. hy_oas_zscore      — inverted HY OAS z-score (tight = risk-on)
  s5. vix_regime         — inverted VIX level + momentum (low/falling = risk-on)

INFLATION composite (→ TIP vs nominal):
  s6. breakeven_roc      — 3m rate of change of 10Y breakeven
  s7. cpi_momentum       — 12m CPI YoY change (realised inflation)
...
def compute_all_macro(macro: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns a daily DataFrame with three composite signals in [-∞, +∞]:
      duration_z  : positive → steep curve / falling rates → favour TLT/IEF
      credit_z    : positive → tight spreads / low VIX → favour LQD/HYG
