...
Identical to V2 pipeline but uses the V2b config (expanded universe).
"""
import os
import logging
import pandas as pd
...
def _cache_path() -> str:
    ...
    return os.path.join(config.DATA_DIR, "sector_v2b_prices.csv")
...
def fetch_prices(start: str = None, force: bool = False) -> pd.DataFrame:
    ...
    prices.to_csv(path)
    logger.info("Fetched V2b sector prices: %d tickers × %d days", len(tickers), len(prices))
    return prices
...
def load_all(force: bool = False) -> pd.DataFrame:
    ...
    if age > 3:
        logger.warning("V2b sector price data last date is %s (%d days ago)",
                       prices.index[-1].date(), age)
    else:
        logger.info("V2b sector price data: %s → %s  (%d tickers)",
