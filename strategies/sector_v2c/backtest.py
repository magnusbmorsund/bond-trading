"""
V2c sector rotation backtest engine.

V2c extends V2b with:
  • Wider cross-asset universe — adds TLT/IEF/HYG (bonds), EFA/EEM/EWJ/EWZ/INDA
    (international equity), PDBC (broad commodities) so there is always something
    to rotate into during equity bear markets / rate-hike regimes.
  • Cluster caps — portfolio.py enforces per-group limits so the strategy cannot
    hold 3 gold miners + 2 green-energy ETFs as its entire "diversified" portfolio.

Trailing stops, vol-target, and DD overlay are identical to V2b.
"""
import logging
import pandas as pd
import numpy as np
...
from strategies.sector_v2c.signals   import (
    composite_score, mom_12m, rolling_vol, spy_regime,
    resample_to_period_end, rolling_corr_at_dates,
)
from strategies.sector_v2c.portfolio import build_weight_series
