"""
V2b sector rotation backtest engine.

Key differences from V2:
  • Wider ETF universe — adds GLD (gold), ITA (defense), XBI (speculative
    biotech), TAN (solar), KWEB (China internet) for genuine supercycle
    diversification across uncorrelated themes.
  • Configurable rebalance frequency — REBALANCE_FREQ in config controls
    how often target weights are rebuilt (default "W" = weekly vs V2 "ME").
  • Daily adaptive trailing stops are unchanged — still fires intraday,
    wider for confirmed supercycles (same STOP_TACTICAL / STOP_SUPERCYCLE).

All other mechanics (composite score, inv-vol weighting, vol-target,
DD overlay) are identical to V2.
"""
import logging
import pandas as pd
import numpy as np
...
from strategies.sector_v2b.signals   import (
    composite_score, mom_12m, rolling_vol, spy_regime, resample_to_period_end,
)
from strategies.sector_v2b.portfolio import build_weight_series
...
def run(prices: pd.DataFrame) -> dict:
    """
    Run V2b sector rotation backtest.

    Rebalance frequency is read from config.REBALANCE_FREQ
    ("W" = weekly, "ME" = monthly). Trailing stops always run daily.
    """
    rebal_freq = getattr(config, "REBALANCE_FREQ", "ME")
    ...
    weights = build_weight_series(score_r, vol_r, regime_r)
