"""
Shared signal functions for sector rotation strategies (V2b–V2e).

Each versioned signals.py is a thin wrapper that imports these functions and
binds its own config module — preserving the Optuna setattr() patching contract.
"""
import pandas as pd
import numpy as np
...
def _weighted_blend(pairs: list) -> pd.DataFrame:
    ...
    During warmup when long lookbacks aren't available, shorter lookbacks carry
    full weight rather than poisoning the composite with NaN.
    """
    num, den = None, None
    for w, m in pairs:
        if w <= 0:
...
def composite_score(prices: pd.DataFrame, config) -> pd.DataFrame:
    ...
    w_24m = getattr(config, "W_SLOW_24M", 0.0)
    w_36m = getattr(config, "W_SLOW_36M", 0.0)
...
def rolling_vol(prices: pd.DataFrame, config) -> pd.DataFrame:
    return prices.pct_change().rolling(config.VOL_WINDOW).std() * np.sqrt(252)


def spy_regime(prices: pd.DataFrame, config) -> pd.Series:
