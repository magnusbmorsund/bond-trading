"""
Shared portfolio builder for sector rotation strategies (V2b–V2e).

Handles both the simple top-N case (V2b, no CLUSTER_CAPS) and the
cluster-capped + correlation-filtered case (V2c/V2d/V2e) via getattr fallbacks.
"""
import logging
import pandas as pd
import numpy as np
...
def _inv_vol_weights(etfs: list, vol_row: pd.Series, config) -> dict:
    inv = {}
    for e in etfs:
...
def _apply_weight_caps(weights: dict, config) -> dict:
    if not weights:
        return {}
    w = dict(weights)
    for _ in range(20):
...
def _select_top_n(scores: dict, n: int, config, corr_matrix=None) -> list:
    """Select top-N ETFs by score, with optional cluster caps + correlation filter."""
    cluster_caps   = getattr(config, "CLUSTER_CAPS", {})
    etf_to_cluster = getattr(config, "ETF_TO_CLUSTER", {})
    ...
    selected = []
    for etf, _ in ranked:
        cluster = etf_to_cluster.get(etf)
        if cluster is not None:
            cap = cluster_caps.get(cluster, 999)
...
def build_weights(
    ...
    top_n = _select_top_n(scores, config.N_POSITIONS, config, corr_matrix=corr_matrix)

    raw_w = _inv_vol_weights(top_n, vol_row, config)
    if not raw_w:
        w[config.CASH_ETF] = 1.0
        return w

    capped = _apply_weight_caps(raw_w, config)
    if not capped:
        w[config.CASH_ETF] = 1.0
        return w
